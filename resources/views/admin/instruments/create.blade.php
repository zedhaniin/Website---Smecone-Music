...
@section('page-title', 'Tambah Alat Musik')
@section('page-description', 'Tambahkan alat musik baru ke inventaris.')

@section('content')
    <div class="mx-auto max-w-2xl">
        <div class="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm">
            <form method="POST" action="{{ route('admin.instruments.store') }}" enctype="multipart/form-data" class="flex flex-col gap-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm transition-all focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 focus:outline-none"
                              placeholder="Deskripsi singkat tentang alat musik">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
